...
`lost_response_resends` 就是**每条命令允许的最大重发次数**（默认 0 = 关闭）。
...
本功能走 esptool 现有的配置系统，**不需要环境变量**。在工程目录放一个 `esptool.cfg`：
...
- **默认关闭，正常芯片零影响。** 仅在确认链路不稳时才设置 `lost_response_resends > 0`。
  - 开启后，对**故意探测不支持命令**的正常路径（如 ESP8266/初代 ESP32 每次连接都会被拒的 `GET_SECURITY_INFO`）会引入少量额外往返与延时，这也是默认设为 0 的原因。
